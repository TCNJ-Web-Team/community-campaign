...
function rt() {
  ...
  const i = t.params,
    ...
    d = R(n, `.${t.params.slideClass}, swiper-slide`),
      ...
      if (i.slidesPerView === "auto") {
        ...
        if (
          ...
          if (B && B === "border-box") P = D + K + V;
            ...
            const { clientWidth: Be, offsetWidth: Re } = E;
            P = D + j + ie + K + V + (Re - Be);
          }
        }
    ...
    (r = parseInt(t.clickedSlide.getAttribute("data-swiper-slide-index"), 10)),
      e.centeredSlides
        ? n < t.loopedSlides - s / 2 ||
          ...
          ? (t.loopFix(),
            (n = t.getSlideIndex(
              R(i, `${o}[data-swiper-slide-index="${r}"]`)[0]
            )),
            ce(() => {
              t.slideTo(n);
            }))
...
function At(t) {
  ...
  const n = () => {
      R(s, `.${i.slideClass}, swiper-slide`).forEach((u, m) => {
...
function jt(t) {
  ...
  if (r.noSwiping && (m ? Rt(u, a) : a.closest(u))) {
    e.allowClick = !0;
    return;
  }
...
let he = class k {
  ...
  getSlideIndex(e) {
    const { slidesEl: i, params: s } = this,
      n = R(i, `.${s.slideClass}, swiper-slide`),
      r = ve(n[0]);
    return ve(e) - r;
  }
...
function ye(t) {
  ...
  function a(c) {
    ...
    if (
      ...
      for (let w = 0; w < m; w += 1)
        R(d.slidesEl, `[data-swiper-slide-index="${e.realIndex + w}"]`).forEach(
          (f) => {
            f.classList.add(p);
          }
        );
